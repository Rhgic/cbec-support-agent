...
|---|---|---|---|---|
| `auto_solve_rate` | action=auto_send 占比 | `eval_e2e`（n=160） | **35.0%**（56/160）——按意图：product 18/21、return 28/57、logistics 10/60、other 0/22 | ≥0.70 |
| `intent_accuracy` | 意图分类正确率 | `eval_rules` + LLM 层抽检 | 规则命中 **46.9%**（75/160），命中内准确率 **100%**（75/75）；整体待 LLM 层 | ≥0.85 |
| `retrieval_recall@5` | 分语种召回率 | `eval_retrieval`（n=112） | ✅ en 39/39、es 37/37、id 36/36，均 **100%**（⚠️ 库仍仅 15 chunk，偏乐观） | en≥0.90 / es≥0.85 / id≥0.80 |
| `grounded_rate` | 有据回答率（有引用且引用真实） | `eval_e2e`（n=160） | **84.4%**（108/128，分母＝走到生成的工单）；编造 URL 检出 100% | ≥0.95 |
| `risk_recall` | 高风险拦截召回 | `eval_risk`（n=100） | ✅ **100%**（47/47）；误升 high **5.7%**（3/53） | ≥0.95 |
| `misjudge_rate` | 规则命中样本内误判率 | `eval_rules`（n=160） | **0%**（0/75，P3 已达到 ≤5%） | ≤0.05 |

> 2026-07-26 P0 扩样基线：工单 160 条（60 条带可查种子订单号）、risk 100、
> retrieval 112、adversarial 84。阈值 0.45 下，误拒率 **0.9%**、对抗暴露率 **3.6%**；
> 0.50 下分别为 8.9% / 0%。当前保留 0.45，并由引用校验与风险闸门承接第二层防线。
> 数据仍为合成样本，接入真实工单后必须重跑。
...
**旧集天花板 ≈ 36%，当时为 28%。** 2026-07-26 已新增 60 条三语种带单号工单，
订单号全部落在 `seed_orders.py` 可查的 `CBEC202400000`–`CBEC202400011` 范围。
全量 n=160 真 LLM 复测后，`auto_solve_rate` 为 **35.0%**；带单号子集为
**25/60 = 41.7%**，原 100 条无单号子集本轮为 **31/100 = 31.0%**。
这证明「查单 → 拿订单状态 → 有据回答」已进入评测主链路，同时也暴露了下一处真实短板：
带单号子集 `grounded_rate` 仅 72.5%（37/51）；全量共有 20 条生成结果因缺引用被拦，
全量 `grounded_rate` 从旧基线 92.2% 降到 84.4%。这是扩样揭示的问题，不应通过删样本掩盖。
...
### P0 数据地基（前置，阻塞一切验收）  ✅ 交接目标已完成（2026-07-26）
- **现状**：工单 160 条（en 54 / es 53 / id 53，含 60 条带单号工单）；
  risk 100、retrieval 112、adversarial 84。
- **问题**：`intent_accuracy`/`risk_recall`/`retrieval_recall` 全是种子数字，无法证明系统真能用。
- **方案**：
  0. **（已完成）`tickets_*.jsonl` 补带订单号工单**——每语种 20 条，共 60 条，
     覆盖 logistics / return 并使用 12 个可查种子订单号。
  1. 本轮先扩至 160 条并出完整基线；每语种 **≥100 条**仍作为接入真实工单前的后续规模目标。
...
- **改动文件**：`datasets/*.jsonl` + 可重复执行的 `scripts/expand_eval_datasets.py`。
- **验收**：✅ 6 个指标已有扩样基线；原始端到端结果见 `runs/expanded_160.json`。
...
> ✅ **可核验证据自检（2026-07-29）**：生成结果新增 `support`，要求模型同时返回来源和从证据中逐字复制的 quote；
> 后端只接受能在本次 chunk / tool result 中精确找到的引文。模型漏填 `citations` 但 support 真实时可安全恢复来源；
> URL 存在但 quote 不可核验、或回复出现证据中不存在的数字事实时，统一升 high 转人工。
> ⏳ **正式指标待重跑**：全量单测 178 通过；本机 Docker/PostgreSQL 未启动，真实 LLM 快速回归未产生新基线，
> 因此 `grounded_rate` 仍保留 84.4%，不提前宣称达到 95%。分语种语气模板仍未做。
...
- **方案**：
  ...
  2. **有据性自检**：✅ 已实现逐字 support quote 校验 + 数字事实核对；出现"无中生有"的事实 → 升 high 转人工。
...
### P3 分类鲁棒性
> ⏳ **规则误判已降至目标内（2026-07-29）**：针对扩样集暴露的借词语种误判、
> `return shipping` 被物流规则抢先、换尺码被产品规则抢先、致谢/改地址被误当物流等问题，
> 增加强语种特征、退换货优先表达与规则旁路。规则层仍坚持“宁可交给 LLM，不强行误判”。
> 当前扩样集实测：coverage 75/160 = 46.9%，命中内 intent 误判 0/75，lang 误判 0/75。
> ✅ **few-shot + 输出边界校验已完成（2026-07-29）**：规则未命中时使用紧凑的三语种边界示例，
> 重点区分换尺码/产品咨询、致谢/物流查询；非法 lang/intent 或异常 confidence 一律按低置信度 other 转人工。
> 规则/LLM 影子一致性仍未做；整体 intent accuracy 需恢复真实 LLM 环境后复测。
...
### P5 人工反馈回流（让系统"越用越好"）  ✅ 离线回流已完成（2026-07-29）
- **现状**：审核接口已支持 `corrected_lang` / `corrected_intent` / `corrected_risk_level`；
  `scripts/export_feedback.py` 可把明确的人工纠正导出为分类、风险和回复归因候选样本。
- **方案**（轻量，不搞自动重训）：
  1. review 的 `failure_tags` + 修正后的 `final_reply` 定期导出成候选标注样本。
  2. 意图被人工改判的工单 → 转成 `tickets_{lang}.jsonl` 新样本 → 反哺规则表/few-shot 示例。
  3. 高风险漏判案例 → 转成 `risk_labeled.jsonl` → 反哺风险规则。
- **安全边界**：只导出 `masked_text`，绝不导出 raw_text；候选文件先落 `runs/feedback_export/`，
  人工复核后才合并到 `datasets/`，不做线上自动学习。
- **改动文件**：`scripts/export_feedback.py`、`routers/review.py`、review schema/model、迁移 `0006_review_corrections.py`。
...
| **U2** | P2 生成有据性（引用校验+自检+语气） | U1 | grounded_rate≥0.95 ⏳ 引用/证据/数字校验已上，语气与正式复测待做 |
| **U3** | P3 分类（扩词+few-shot+一致性） | U0 | ⏳ 扩词/few-shot/边界校验完成，一致性与真实 LLM 复测待做 |
| **U4** | P4 风险（归一化+豁免+敏感动作） | U0 | ✅ risk_recall 100%（合成集，已完成） |
| **U5** | P5 反馈回流（导出+反哺） | U1–U4 | ✅ 离线导出链路完成；月增≥50 须真实使用后统计 |
