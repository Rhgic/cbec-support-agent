...
|---|---|---|---|---|
| `auto_solve_rate` | action=auto_send 占比 | 端到端批量跑 | ⚠️ **50%**（6/12，真实 LLM）——按意图极不均衡：product **6/6**、return **0/3**、logistics **0/3**，根因见下方「首要杠杆」 | ≥0.70 |
...
| `grounded_rate` | 有据回答率（有引用且引用真实） | 端到端批量跑 | ✅ **91.7%**（11/12，真实 LLM）；编造 URL 检出 100% | ≥0.95 |
...
### 🎯 首要杠杆：`tools` 对通用政策问题的过度拦截（未决，需业务拍板）

批量实测暴露一个极清晰的模式：**product 意图 6/6 自动放行，return / logistics 各 0/3 全部转人工。**

根因不是模型答得差——这 12 条的回复都有真实引用、内容准确。而是 `tools` 节点对 return/logistics
会尝试查订单，**通用政策问题里本来就没有订单号** → 记 `tool_errors` → 触发风险规则 4 → 强制 high。
（bug #11 已修掉 product 这一路，return/logistics 同类问题仍在。）

例：「跨境物流到加拿大要多久？」「不喜欢的商品几天内可退？」——知识库能完整回答，无需任何订单数据，
却因"没提取到订单号"被判高风险。**这是当前压住自动解决率的最大单一因素**：修掉后 50% 有望接近 92%。

但它不是纯 bug，是**风险策略的取舍**，需要业务定义：

| 方案 | 效果 | 代价 |
|---|---|---|
| A. 无订单号 ≠ 错误（视为"不适用"） | 自动解决率大幅提升 | 「我的订单在哪」这类**确实需要订单数据**的问题，会被自动回一条通用政策答复 |
| B. 按"问题是否需要订单事实"区分 | 兼顾两者 | 需要一条判定规则（关键词/LLM），有维护与误判成本 |
| C. 维持现状 | 最保守，零资损风险 | 自动解决率锁死在 ~50% |

> 倾向 B，但**该由业务定**：这是"少雇几个客服"与"绝不答错具体订单"之间的定价，不该由实现者替业务决定。
